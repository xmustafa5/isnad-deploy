...
import { RealEstateItem, RealEstateResponse } from '@/types/realEstateType';
import { useQuery } from '@tanstack/react-query';
import Link from 'next/link';
import React, { useState, useRef, useEffect, useCallback } from 'react'
export default function WrapperRealEstate({ initialData }: { initialData: RealEstateResponse }) {
    ...
    return (
        ...
        <div className='flex justify-between shrink-0 gap-6'>
            <div className='filter hidden  relative xl:flex w-[430px] pt-4 flex-col items-center shrink-0  rounded-t-[40px] rounded-b-0 border border-[rgba(255,255,255,0.10)] bg-[rgba(8,24,47,0.10)] backdrop-blur-[68.5px]'>
                <div className='content filter overflow-x-hidden overflow-y-auto h-[calc(99.3vh-200px)] flex flex-col px-4 items-start gap-6 flex-shrink-0 self-stretch'>
                ...
                <div className='flex flex-col w-full gap-4'>
                    ...
                    <div className='flex flex-col gap-[24px]'>
                        <div className='flex overflow-y-auto max-h-[calc(95.3vh-200px)] py-4 items-center content-start gap-[24px] self-stretch justify-center xl:justify-start flex-wrap'>
                            {data?.items?.map((item: RealEstateItem) => (
                                <CardProperty key={item.geoid} item={item} />
                            ))}
                        </div>
                    </div>
                </div>
...
function CardProperty({ item }: { item: RealEstateItem }) {
    ...
    return (
        <Link
            ...
            >
                ...
                <p className="text-text-main text-[18px] font-normal leading-[140%]">{item?.price}</p>
            ...
            <div className="flex flex-col items-end gap-[8px] flex-1" >
                <div className="flex flex-col justify-center items-end gap-[8px] self-stretch">
                    <p className='text-text-main w-full typography-body-16-medium'>{item?.name?.ar}</p>
                    <div className="flex justify-end items-center gap-[8px] self-stretch">
                        ...
                        <p className='text-text-gray2 w-full  typography-body-14-light'>{item?.governorate?.name?.ar}</p>
                    </div>
                </div>
                <div className="flex  items-center content-center gap-[8px] self-stretch flex-wrap">
                    <div className="flex justify-center items-center gap-[4px] px-[16px] py-[6px] rounded-[10px] border border-[rgba(234,28,28,0.05)] bg-opacity-secondary-10 backdrop-blur-[65px]">
                        ...
                        <p className='text-text-main typography-body-14-light'>{item?.area}م</p>
                    </div>
                </div>
            </div>
        </Link>
