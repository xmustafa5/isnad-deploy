...
export default function Input({ label = 'الحد الأدنى', subLabel = 'متر', onChange }: { label?: string, subLabel?: string, onChange?: (value: string) => void }) {
    return (
        <div className="flex w-[175px] flex-col items-end gap-[8px]">
            ...
            <div className="flex pl-[16px] items-center gap-[8px] self-stretch rounded-[12px] border border-stroke-white-5 bg-[rgba(13,31,57,0.05)]">
                ...
                <input type="text" className='text-text-gray2 typography-subtitle-18-medium w-full focus:outline-none' onChange={(e) => onChange?.(e.target.value)} />
            </div>
        </div>
    )
}
